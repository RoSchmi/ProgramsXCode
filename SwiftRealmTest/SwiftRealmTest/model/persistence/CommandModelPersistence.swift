...
public class CommandModelPersistence {
	let model: Command
	let realm: Realm
	...
	init(model: Command) {
		self.model = model
		self.realm = CommandModelPersistence.initRelam()
	}
	...
	func update(_ command: Command) {
		...
		if let setting = settings.first {
			do {
				try realm.write {
                    setting.dId = command.ID
                    setting.dTitle = command.title
                    setting.dExplanation = command.explanation
                    setting.dQos = command.qos
